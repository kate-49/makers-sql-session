...
class CountryData
  ...
  def self.highest_gni_per_capita_in_asia
    result = DatabaseConnection.query("SELECT * FROM countries ORDER BY gni DESC LIMIT 1;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end

  def self.third_lowest_gni_per_capita_in_south_america
    result = DatabaseConnection.query("SELECT * FROM countries WHERE continent = 'South America' ORDER BY gni DESC LIMIT 3;")
    result.map{ |country| CountryData.new(country['name'], country['continent'], country['population'], country['density'], country['gni']) }
  end
